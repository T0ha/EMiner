...
%                  {<<"id">>, <<"json">>}
%                 ]} ->
%            %error_logger:info_msg("Got work: ~p ~n", [Res]), %integer_to_list(Target, 16)]),
%            <<Data:152/bytes, _/bytes>> = D = proplists:get_value(<<"data">>, Res),
%            <<Target:256/little-integer>> = hex_to_bin(proplists:get_value(<<"target">>, Res)),
%            %Target = binary:decode_unsigned(<< <<255>> || _ <- lists:seq(0, 30)>>),
%            error_logger:info_msg("Got work: ~p target ~p~n", [D , Target]), %integer_to_list(Target, 16)]),
